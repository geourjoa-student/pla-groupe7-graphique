...
public class Partie extends Parent{
		...
		this.setOnKeyPressed(new EventHandler<KeyEvent>(){
	        public void handle(KeyEvent ke){
				...
				default:
					...
					System.out.println("2 -> rÈcolter");
					System.out.println("3 -> soigner");
					System.out.println("4 -> convertir");
					System.out.println("5 -> crÈer unitÈ");
		...
		for(int i=0;i<autoJ1.size();i++){
			...
			if(autoTempo.getRole() == 1) {
				...
				for(int j=0;j<tableauTempo.length;j++){
					for(int k=0;k<tableauTempo[j].length;k++){
						...
						// Le premier automate s'affiche 2 cases ÔøΩ droite du hÔøΩros
			...
			if(autoTempo.getRole() == 2) {
				...
				for(int j=0;j<tableauTempo.length;j++){
					for(int k=0;k<tableauTempo[j].length;k++){
						tableauTempo[j][k].setProprietaire(joueur1);
						// Le premier automate s'affiche (largeur de l'auto +1) cases ÔøΩ gauche du hÔøΩros
			...
			if(autoTempo.getRole() == 3) {
				...
				for(int j=0;j<tableauTempo.length;j++){
					for(int k=0;k<tableauTempo[j].length;k++){
						tableauTempo[j][k].setProprietaire(joueur1);
						// Le premier automate s'affiche ÔøΩ la moitiÔøΩ de la map - la moitiÔøΩ de la hauteur du auto, et tout ÔøΩ gauche +1
	...
	private void creerDecor(String nomFichierAutomates1, String nomFichierAutomates2) {
		...
		//On rempli le d√©cror d'herbe et de ligne de champs, d'arbre, 
			...
			if (personnage.estVivant()) {
				...
				switch (actionAfaire) {
					...
					case ATTAQUER_BATIMENT:
						...
						break;
					case CREER_UNITE:
						//On ne peut ajouter les personnages qu'apr√®s la fin de l'it√©ration : err java.util.ConcurrentModificationException
	...
	private List<Personnage> ordonnanceur() {
		//Ordonnanceur simpliste, on peut l'am√©liorer ici
		return personnages;
	}
